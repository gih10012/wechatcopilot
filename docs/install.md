...
Persist the driver variables before installing the user service. Shell
`export` commands affect only the current shell; `daemon install` intentionally
does not copy driver paths, URLs, image references, or digests out of that
shell. Create the service's private environment file as a regular, non-symlink
file and edit it with the values independently verified for this host:

```bash
config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/wechatcopilot"
driver_env="$config_dir/environment"
install -d -m 0700 "$config_dir"
test ! -L "$driver_env"
if [ ! -e "$driver_env" ]; then
  install -m 0600 /dev/null "$driver_env"
fi
test -f "$driver_env"
chmod 0600 "$driver_env"
"${EDITOR:-vi}" "$driver_env"
```

Use systemd `EnvironmentFile` assignments without `export`. A complete
two-driver example is:

```ini
WECHATCOPILOT_WECHAT_IMAGE="wechatcopilot/wechat-runtime:local"
WECHATCOPILOT_WECHAT_APPIMAGE="/absolute/path/to/WeChat.AppImage"
WECHATCOPILOT_WECHAT_APPIMAGE_SHA256="64_HEX_CHARACTERS"
WECHATCOPILOT_WECOM_REDROID_IMAGE="redroid/redroid:VERSION@sha256:REVIEWED_DIGEST"
WECHATCOPILOT_WECOM_APK_URL="https://APPROVED_TENCENT_HOST/path/to/wecom.apk"
WECHATCOPILOT_WECOM_APK_SHA256="64_HEX_CHARACTERS"
WECHATCOPILOT_WECOM_APK="/absolute/path/to/wecom.apk"
WECHATCOPILOT_WECOM_COMPANION_APK="/absolute/path/to/wechatcopilot-companion.apk"
```

Do not put login URLs, QR data, SMS codes, cookies, account credentials, or
companion tokens in this file. Load the same non-secret assignments into the
current trusted shell for preflight, then install the daemon only after
`doctor` passes:
...
The generated unit reads this optional driver file first, then the required
mode-`0600` state-mount and swap-policy files generated by `daemon install`.
Editing the driver file later requires `wechatcopilot daemon restart`; rerun
`daemon install --force` only when the generated unit or pinned mount/swap
...
Omit `--lan` when the browser is on the same host. With `--lan`, omit `--lan-address` to prefer the default-route LAN interface, or provide an exact RFC1918 address already assigned to an eligible local interface. The command rejects public, wildcard, loopback, container-bridge, and unassigned addresses. It prints a one-time URL and expiry. Complete QR scan, phone confirmation, verification-code entry, or explicitly advertised official-client onboarding actions directly on that page. On a supported WeCom login-method screen, the page first asks you to confirm the displayed agreements and then, after re-reading the official UI, separately asks whether to continue with the current WeChat identity. The image is never cropped from an unverified accessibility rectangle; open it at full size when needed. A successful page remains available for about 60 seconds to display completion, then the login listener closes.
