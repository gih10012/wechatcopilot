...
The page may advertise a fixed, high-risk onboarding action such as accepting the privacy policy displayed by the official WeCom client. On the verified WeCom login-method screen, accepting the displayed login agreements and selecting `Continue with WeChat` are two separate confirmations; only one is advertised at a time and the official screen is freshly revalidated between them. Agreement selection uses a dedicated check-only action that rejects an already checked control, so a retry cannot silently withdraw consent. Only that bearer-scoped page can invoke these actions, and only after the user confirms each one there. Before acting, the driver rechecks the official package, an allowlisted foreground Activity, multiple visible page markers, unique semantic targets, and the current accessibility sequence. Once the official client accepts an action, its ID is consumed even if post-action observation times out, preventing ambiguous retries. Actions expose neither coordinates nor node IDs and are absent from the daemon API, CLI, and MCP. Verification-code submission is likewise accepted only while the current official screen advertises one SMS input. Account-risk and device-verification screens never expose an onboarding action.
